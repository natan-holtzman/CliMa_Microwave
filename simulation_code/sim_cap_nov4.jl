...
function run_sim(vcmax_par, k_rel_crit, k_plant, k_soil, z_soil, istart, N, smc0, slope_index,use_flux,k_weibB,k_weibC,storage_mult)
...
node = create_moflux_node(vcmax_par, k_plant, z_soil, smc0, storage_mult);
...
for i in eachindex(df.Day)
	...
	for subI2 in 1:subIter2
		...
		do_soil_nss_drain!(node, FT(df.RAIN[i])/subIter2, deltaT/subIter2, k_soil, slope_index)		
