...
function update_pk_old!(plant_hs)

	#plant_hs = node.plant_hs

	for can_i in 1:plant_hs.n_canopy
		iLeaf = plant_hs.leaves[can_i]		
		iLeaf.p_storage = (iLeaf.v_storage/iLeaf.v_maximum .- 1) * iLeaf.pv.slope
		iLeaf.p_storage = iLeaf.p_storage*0 .+ iLeaf.p_storage[1]
		iLeaf.p_element = iLeaf.p_element*0 .+ iLeaf.p_storage[1]
		iLeaf.p_leaf = iLeaf.p_storage*1
		iLeaf.p_ups = iLeaf.p_storage[1]
		iLeaf.k_element = exp.(-(iLeaf.p_element ./ -iLeaf.vc.b) .^ iLeaf.vc.c) * iLeaf.k_sla
		
		iBranch = plant_hs.branch[can_i]
		iBranch.p_storage = (iBranch.v_storage ./ iBranch.v_maximum .- 1) * iBranch.pv.slope
		iBranch.p_storage = iBranch.p_storage*0 .+ iBranch.p_storage[1]
		iBranch.p_element = iBranch.p_storage*1
		#iBranch.p_ups = iBranch.p_storage[1]
		iBranch.k_element = exp.(-(iBranch.p_element ./ -iBranch.vc.b) .^ iBranch.vc.c) * iBranch.k_max
	end

	tissue =  plant_hs.trunk
	tissue.p_storage = (tissue.v_storage ./ tissue.v_maximum .- 1) * tissue.pv.slope
	tissue.p_storage = tissue.p_storage*0 .+ tissue.p_storage[1]
	tissue.p_element = tissue.p_storage*1
	...
	for root_i in 1:plant_hs.n_root
		...
		iRoot.p_storage = (iRoot.v_storage ./ iRoot.v_maximum .- 1) * iRoot.pv.slope
		iRoot.p_storage = iRoot.p_storage*0 .+ iRoot.p_storage[1]
		iRoot.p_element = iRoot.p_storage*1
		iRoot.k_element = exp.(-(iRoot.p_element ./ -iRoot.vc.b) .^ iRoot.vc.c) * iRoot.k_max
	end

end

#=
function p_from_vol_v2(vstore::FT,vmax::FT,pvslope::FT)
	return (vstore/vmax .- FT(1)) * pvslope
end
=#
