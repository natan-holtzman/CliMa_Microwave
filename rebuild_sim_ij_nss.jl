...
function run_sim(vcmax_par, p_crit, k_plant, k_soil, b_soil, p20, z_soil, n_soil, istart, N, smc0, kstore, buffrate)
...
myKrefill = FT(1e-4);
for i_can in 1:n_canopy
	plant_hs.leaves[i_can].pv.k_refill = myKrefill
	plant_hs.branch[i_can].pv.k_refill = myKrefill
end

plant_hs.trunk.pv.k_refill = myKrefill;
	
for i_root in 1:n_root
	plant_hs.roots[i_root].pv.k_refill = myKrefill
end
=#
...
for i in eachindex(df.Day)
	...
	if i == 1
		...
		#=
		update_Kmax!(node, FT(100))
		for init_iter in 1:10000
			update_PVF!(node.plant_hs,FT(1));
		end
...
=#
	...
	#=
	hs2 = deepcopy(plant_hs);
	update_PVF!(hs2,deltaT/subIter2/subIter3);
	if hs2.leaves[1].p_leaf >= 0 | isnan(hs2.leaves[1].p_leaf)
		subIter3 = 20
	end
	=#
	
	#=
	for iter_s in 1:deltaT
			update_PVF!(node.plant_hs,FT(1));
	end
	=#
	...
	for subI2 in 1:subIter2
		...
		if i >= 1
			for si3 in 1:subIter3
				update_PVF!(node.plant_hs,deltaT/subIter2/subIter3);
			end
