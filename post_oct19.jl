...
sim_res1 = convert_sim(run_sim(FT(22),FT(1.5), FT(4.0), FT(1e-4), FT(2.5), FT(2.0), FT(700),FT(0.45),istart,N, soil_init1));
...
trueTB = get_TB(sim_res1, mean(sim_res1[3],dims=2), 0.0, 1, 1/50);
...
obs_mask[14:(24*3):end] .= 1;

#obs_mask = ones(length(obsH));

obs_mask = obs_mask .== 1;

function log_p_nolabel(a, k0, alpha0, beta0, maxiter, stop_crit)
...
sim_res = convert_sim(run_sim(FT(exp(v.logVcmax)),FT(exp(v.logPcrit)), FT(exp(v.logKmaxPlant)), FT(exp(v.logKmaxSoil)), FT(2.5), FT(2.0), FT(exp(v.logZsoil)), 
			  FT(exp(v.logNsoil)), istart, N, soil_init1));
	...
	optim_res = optim_vod_stages(obsH[obs_mask], obsV[obs_mask],
		...
		p += logpdf(MvNormal(sim_TB[1][obs_mask], noise_var), obsH[obs_mask]);
		p += logpdf(MvNormal(sim_TB[2][obs_mask], noise_var), obsV[obs_mask]);		
...
ll0,vodpar0 = log_p_nolabel(pars,k0,alpha0,beta0,10000,10^-6);
mycov = I*0.01/length(pars);
chain = zeros(niter, nPar*2+11);
...
for j in 1:niter
    ...
    llP,vodparP = log_p_nolabel(parsP,vodpar0[1],vodpar0[2],vodpar0[3],10000,10^-6);
   chain[j,nPar+5] = llP;
   chain[j,(nPar+6):(nPar+10)] = vodparP;
    mhratio = exp(llP - ll0);
    randI = rand();
   chain[j,nPar+11] = randI;
   chain[j,(nPar+12):end] = parsP;
    if randI < mhratio
		ll0 = llP*1;
		pars = parsP[:,1]*1;
		vodpar0 = vodparP*1;
...
ll_init, vodpar_init = log_p_nolabel(Array(x0), k0,alpha0,beta0,10000,10^-6);
...
CSV.write("post_fixsoil_3day_0sens.csv",dfc);
=#


function sample_post(infile, out_suffix, out_folder, fixsoil)

results = CSV.read(infile,DataFrame);

par_results = Array(results[7501:25:end,1:8]);

post_len = size(par_results)[1];

post_psi = zeros(365*24, post_len+1);
post_smc = zeros(365*24, post_len+1);
post_et = zeros(365*24, post_len+1);
post_surf = zeros(365*24, post_len+1);
...
println(i)
	...
	if fixsoil==1
		...
		bsoil = FT(exp(v.logBsoil))
		p20 = FT(exp(v.logP20))
	end
        sim_res = convert_sim(run_sim(FT(exp(v.logVcmax)),FT(exp(v.logPcrit)), FT(exp(v.logKmaxPlant)), FT(exp(v.logKmaxSoil)), bsoil, p20, FT(exp(v.logZsoil)),
                          FT(exp(v.logNsoil)),istart,N,soil_init1));



        post_psi[:,i] = mean(sim_res[3],dims=2);
        post_smc[:,i] = sim_res[2][:,4];
        post_et[:,i] = sim_res[1].ETmod;
	post_surf[:,i] = sim_res[2][:,1];

end

i = post_len+1
post_psi[:,i] = mean(sim_res1[3],dims=2);
post_smc[:,i] = sim_res1[2][:,4];
post_et[:,i] = sim_res1[1].ETmod;
post_surf[:,i] = sim_res1[2][:,1];

CSV.write(string(out_folder, "/psi_",out_suffix, ".csv"), DataFrame(post_psi));
CSV.write(string(out_folder, "/rzsm_",out_suffix, ".csv"), DataFrame(post_smc));
CSV.write(string(out_folder, "/ET_",out_suffix, ".csv"), DataFrame(post_et));
CSV.write(string(out_folder, "/surfsm_",out_suffix, ".csv"), DataFrame(post_surf));

end


sample_post("post_par_obs3day_8par.csv", "8par_3day", "post_oct20",0)
sample_post("post_8par_obs3day_0sens.csv", "8par_3day_0sens", "post_oct20",0)
...
#=
a = mean(c1[2000:end,:],dims=1);
v = LVector(logVcmax=a[1], logKplant=a[2], logWeib_b=a[3], logSoilB=a[4], logSoilP=a[5], logPvs=a[6]);
sim_res2 = run_sim(FT(exp(v.logVcmax)),FT(exp(v.logWeib_b)), FT(exp(v.logKplant)), FT(1e-3), FT(exp(v.logSoilB)), FT(exp(v.logSoilP)));

plot(sim_res1[1].leafpot); plot!(sim_res2[1].leafpot)
=#
