...
mypv = PVCurveLinear{Float32}(20f0, FT(1e-4));
...
for i in 1:n_canopy
	plant_hs.leaves[i].v_maximum = FT(storage_mult) * plant_hs.leaves[i].v_maximum
	plant_hs.branch[i].v_maximum = FT(storage_mult) * plant_hs.branch[i].v_maximum
end

plant_hs.trunk.v_maximum = FT(storage_mult) * plant_hs.trunk.v_maximum

for i in 1:n_root
	plant_hs.roots[i].v_maximum = FT(storage_mult) * plant_hs.roots[i].v_maximum
end
...
#=
for i_init in 1:1000
	dd1, dd2 = update_cap_mat!(node,FT(1));
end
=#
