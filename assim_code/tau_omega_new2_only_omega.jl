...
function get_TB_2(smc_mod, Tsoil, Tcan, leafpot, laiM, pvs, alpha, beta,omega)
...
function optim_vod_while2(obsHi, obsVi, leafpot, smc_mod, Tsoil, Tcan, laiM, k, alpha, beta, omega, maxiter, stop_crit, step_size)
...
while (itercount < maxiter) & (err_diff > stop_crit)
	...
	total_err_old = total_err_new*1;
        ...
        omega = min(max(0,omega ),0.5);

	
return [k, alpha, beta, omega, total_err_old, itercount];
end

function optim_vod_stages2(obsHi, obsVi, leafpot, smc_mod, Tsoil, Tcan, laiM, k, alpha, beta, omega, maxiter, stop_crit,step_size, nskip)
	stage1 = optim_vod_while2(obsHi[1:nskip:end], obsVi[1:nskip:end], 
    leafpot[1:nskip:end], smc_mod[1:nskip:end], Tsoil[1:nskip:end], Tcan[1:nskip:end], laiM[1:nskip:end],
    k, alpha, beta, omega, maxiter, stop_crit, step_size);
	k1, alpha1, beta1, omega1, e1, i1 = stage1;
	stage2 = optim_vod_while2(obsHi, obsVi, 
    leafpot, smc_mod, Tsoil, Tcan, laiM,
    k1, alpha1, beta1, omega1, maxiter, stop_crit, step_size);
	return stage2;
end
