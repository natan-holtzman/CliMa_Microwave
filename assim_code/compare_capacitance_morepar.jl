...
parnames = ["vcmax","g1","p63","Kplant","Capacitance","Zsoil",
"Terrain_slope","root_dist_factor","canopy_pv","trunk_pv"];
par_indices = [1,7,5,3,6,8,9,10,11,12 ];
...
subplot(1,3,1)
plot(reshape(factor_list4,(nfac,npar)),reshape(tdiff_all,(nfac,npar)),"o-",label=parnames)
...
subplot(1,3,2)
plot(reshape(factor_list4,(nfac,npar)),reshape(tdiff_1,(nfac,npar)),"o-",label=parnames)
xscale("log")
ylim(0,0.9)
title("1 AM/PM")
xticks(factor_list,round.(factor_list,digits=2))
...
subplot(1,3,3)
plot(reshape(factor_list4,(nfac,npar)),reshape(tdiff_6,(nfac,npar)),"o-",label=parnames)
xscale("log")
ylim(0,0.9)
title("6 AM/PM")
xticks(factor_list,round.(factor_list,digits=2))
...
figure()
#bar(collect(1:10) .+ 0.1,tdiff_1[1:2:end])
#bar(collect(1:10) .- 0.1,tdiff_1[2:2:end])
bar(1:10, 0.5*(tdiff_6[1:2:end]+tdiff_6[2:2:end]))
xticks(1:10,parnames)