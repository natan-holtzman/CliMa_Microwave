...
function run_sim_2(vcmax_par::FT, k_frac::FT, k_plant::FT, k_soil::FT, z_soil::FT, weibB::FT, weibC::FT, vol_factor::FT, g1::FT, pot1::FT, n_soil::FT)
	m = 1 - 1/n_soil;
	alpha = (FT(0.1) ^ (-1/m) - 1) ^(1/n_soil)/pot1;
	return run_sim_vary(vcmax_par, k_frac, weibB, weibC, k_plant, k_soil, z_soil, istart, N, soil0, vol_factor, 1e-5, 3, df_raw, g1, deltaT, alpha, n_soil);
end

sim_res1 = run_sim_2(FT(22),FT(0.33), FT(2),FT(1e-5), FT(600),FT(4.0),FT(3),FT(1),FT(600),FT(2),FT(2.6));
...
prior_min = [10, 0.01, 0.1,  1e-6, 500, 0.75, 0.75, 0.1,100,1.0,1.0];
prior_max = [120,0.75,  100, 2e-5, 3000, 10, 8, 10,1000,4.0,3.0];
