...
subdir_list = ["oAll_c2/", "o1AMPM_c2/","o6AMPM_c3/"]
...
print(len(leafpost))
print([x.shape for x in leafpost])
...
def get_diurnal_2d(x,nstep):
    ...
    dmean = np.mean(y, axis=0)
    dmean2 = dmean #- np.mean(dmean, axis=0).reshape((1,-1))
    return dmean2

def get_diurnal_1d(x,nstep):
    y = np.reshape(x, (-1, nstep))
    return np.mean(y, axis=0) - np.mean(y)
...
daily_ser = [get_diurnal_2d(x,24) for x in leafpost]
daily_mean = [np.mean(x[:,:-1],axis=1) for x in daily_ser]
daily_25 = [np.quantile(x[:,:-1],0.25,axis=1) for x in daily_ser]
daily_75 = [np.quantile(x[:,:-1],0.75,axis=1) for x in daily_ser]

np.savetxt("DL_means2.csv", np.transpose(np.array(daily_mean)))
np.savetxt("DL_lower2.csv", np.transpose(np.array(daily_25)))
np.savetxt("DL_upper2.csv", np.transpose(np.array(daily_75)))

np.savetxt("DL_true2.csv",np.transpose(daily_ser[0][:,-1]))
