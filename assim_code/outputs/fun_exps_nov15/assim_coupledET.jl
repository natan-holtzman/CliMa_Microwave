...
df_raw = avg2_df(CSV.read(string(PROJECT_HOME,"/data/moflux_land_data_newnames_7.csv"), DataFrame));
df_raw[!,"RAIN"] *= 2;
			...
			if isnan(mean(sim_res[1].leafpot))
				...
				soilsurf = sim_res[2][:,1];
                                optim_res = optim_vod_stages2(obsH[obs_mask], obsV[obs_mask], canpot[obs_mask], soilsurf[obs_mask],
                                                                        tsoil[obs_mask], tcan[obs_mask], laiM[obs_mask], k0, alpha0, beta0, maxiter, stop_crit, 1e-5, 1)
