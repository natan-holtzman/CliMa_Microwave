...
prior_min = [10, 0.75, 0.1, 500, 0.01,  0.01,100,0.1e-6,0.3,0.01,1,1.1];
prior_max = [120,15,  50, 3000, 10, 10,1000,    20e-6,0.5,5,8,1.9];
...
function runAMH(x_init, niter,start_full,burn_len,cov_init)
	...
	ll0 = p;
	mycov = 1*cov_init; #I*0.01/length(pars);
...
prev_sol = Array(CSV.read(init_file,DataFrame));

a01 = prev_sol[end,1:NPAR];
cov01 =  (1-10^-6)*cov(prev_sol[9000:end,1:NPAR])  + 10^-6*I;

c1 = runAMH(a01,5000,1000,500,cov01);

dfc = DataFrame(c1);
CSV.write(string(outdir,"post_par_next.csv"),dfc);
