...
function do_assim(obs_mask, norm_factor, outdir, vodA, vodB, vodC,vodO,obs_file,init_file)
...
function log_p_nolabel(a, k0, alpha0, beta0,omega0, maxiter, stop_crit, error_var, old_TB, old_par)
			...
			if isnan(mean(sim_res[1].leafpot))
				...
				soilsurf = sim_res[2][:,1];
                                ...
                                optim_res = optim_vod_stages2(obsH[obs_mask], obsV[obs_mask], canpot[obs_mask], soilsurf[obs_mask],
                                                                        tsoil[obs_mask], tcan[obs_mask], laiM[obs_mask],
 k0, alpha0, beta0,omega0, maxiter, stop_crit, 1e-5, n_to_skip)
                                sim_TB = get_TB_2(soilsurf, tsoil, tcan, canpot, laiM,optim_res[1], optim_res[2],optim_res[3],optim_res[4]);
...
function runAMH(x_init, niter,start_full,burn_len)
	...
	beta0 = exp(rand(Uniform(log(0.01),log(0.5))));
        omega0 = exp(rand(Uniform(log(0.01),log(0.5))));
	...
	p,p0,sim_res0,tb0,optim_res0 = log_p_nolabel(pars,k0,alpha0,beta0,omega0,10000,10^-6, err0, (obsH,obsV),pars);
	ll0 = p;
	mycov = I*0.01/length(pars);
	chain = zeros(niter, nPar*2+2+4);
	...
	for j in 1:niter
		...
		chain[j,(nPar*2+3):end] = optim_res0[1:4];
		...
		p,p0,sim_resP,tbP,optim_resP = log_p_nolabel(parsP,optim_res0[1],optim_res0[2],optim_res0[3],optim_res0[4],10000,10^-6, err0, tb0,pars);
		mhratio = exp(p-p0);
		randI = rand();
		if randI < mhratio
			ll0 = p;
...
perturb_start_rel = opt_sol_rel + 0.05*(rand(NPAR) .- 0.5);
