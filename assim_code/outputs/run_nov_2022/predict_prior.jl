...
par_post = transpose(Matrix(CSV.read("prior_sample_inlog.csv",DataFrame)));
println(size(par_post));
...
post_TWS = zeros(nday*24,npost+1);
...
try
	...
	#post_RZ[:,j] = get_daily(sim_res0[2][:,end],24);
        post_TWS[:,j] = sim_res0[1].ColumnSMC;
	...
	#post_RZ[:,end] = get_daily(sim_res1[2][:,end],24);
        post_TWS[:,end] = sim_res1[1].ColumnSMC;
...
CSV.write(string(outdir,"postSWS.csv"), DataFrame(post_TWS,:auto) );
#CSV.write(string(outdir,"postSWP.csv"), DataFrame(post_TWP) );
CSV.write(string(outdir,"postET.csv"), DataFrame(post_ET,:auto));
CSV.write(string(outdir,"postLeaf.csv"), DataFrame(post_LWP,:auto));
#CSV.write(string(outdir,"postBranch.csv"), DataFrame(post_branch));
#CSV.write(string(outdir,"postTrunk.csv"), DataFrame(post_trunk));
CSV.write(string(outdir,"postGSW.csv"), DataFrame(post_gsw,:auto));
CSV.write(string(outdir,"postGPP.csv"), DataFrame(post_GPP,:auto));
end
