...
#subdir_list = subdir_list[:3]

par_post = []
for x in subdir_list:
    g1 = np.array(pd.read_csv(out_folder+x+"post_par.csv"))[-4000::100,:];
    #if x == "o1AMPM_c3":
    #    g1 *= np.nan
    par_post.append(g1)
...
allpars[:,:,11] *= -1 #it's in terms of log mpa^-1, need to convert to log mpa
...
j0 = 0
fig, ax_all = plt.subplots(5,4,figsize=(12,10))
for i in range(4):
...
for i2 in range(4):
    ...
    for element in vpi.keys():
        if element=="bodies":
            ...
            vpi[element].set_color(colors_list)
    #ax.set_ylim(vod_min[i2],vod_max[i2])
    ax.set_xticks([])
    ax.hlines(true_vod[i2],0,6,color="black")
    #ticklocs = np.array([vod_min[i2],true_vod[i2],vod_max[i2]])
    ax.set_xlim(0.25,5.75)
    #ax.set_yticks(ticklocs)
    ax.set_title(vod_names[i2])
...
plt.tight_layout()
plt.savefig("violin_fix_slope_all.png")
