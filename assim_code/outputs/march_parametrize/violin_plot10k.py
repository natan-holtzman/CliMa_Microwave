...
allpars = np.concatenate((par_post[:,6000::100,:11], par_post2[:,6000::100,:11], par_post3[:,2000::100,:11]), axis=1)
print(allpars.shape)
...
prior_min = [10, 0.75, 0.1, 500, 0.01,  0.01*12,100,0.1,0.3,0.01,1];
prior_max = [120,15,  50, 3000, 10, 10*12,1000,    4,0.5,5,8];
...
for ax in ax_all.ravel()[:11]:#[:7]:
    ...
    ax.violinplot(np.transpose(allpars[:,:,j]))
